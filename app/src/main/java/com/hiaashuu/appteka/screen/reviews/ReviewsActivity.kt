...
class ReviewsActivity : AppCompatActivity(), ReviewsPresenter.ReviewsRouter {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val view = ReviewsViewImpl(window.decorView, adapter)

        presenter.attachView(view)

        if (savedInstanceState == null) {
            analytics.trackEvent("open-reviews-screen")
        }
    }
...
fun createReviewsActivityIntent(
    context: Context,
    userId: Int
): Intent = Intent(context, ReviewsActivity::class.java)
