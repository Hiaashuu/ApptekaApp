...
class ZoomableImageView : ImageView, ViewportUpdateListener {
    ...
    private fun tryFixMinScale() {
        ...
        fun KLine.getVectorDistantPoint(): KPoint {
            val distantPoint = max(imageWidth, imageHeight)
            val dx = p2.x - p1.x
            if (dx == 0f) return KPoint(p1.x, distantPoint)
            val k = (p2.y - p1.y) / dx
        ...
        val diagonals = listOf(
                ...
                KLine(viewportCenter, KPoint(viewport.right, viewport.top)),
                KLine(viewportCenter, KPoint(viewport.right, viewport.bottom))
        ).map { it.p2 to KLine(it.p1, it.getVectorDistantPoint()) }
    ...
    private fun fixTrans() {
        ...
        if (rotationAngle == NO_ROTATION_ANGLE) {
            removeExtraTrans()
        } else {
            val image = getCurrentBounds() ?: return
            val viewportRect = viewport.toKRect()
            if (image.contains(viewportRect)) return
    ...
    private fun getBounds(imageMatrix: Matrix): KRect? {
        ...
        val points = floatArrayOf(
                0f, 0f,
                drawableWidth.toFloat(), 0f,
                drawableWidth.toFloat(), drawableHeight.toFloat(),
                0f, drawableHeight.toFloat()
        )
